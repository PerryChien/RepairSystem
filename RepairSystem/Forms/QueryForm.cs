...
namespace RepairSystem.Forms
{
    public partial class QueryForm : Form
    {
        ...
        private void SetupForm()
        {
            ...
            buttonNext.Click += (s, e) => { if (currentPage < totalPages) currentPage++; LoadQueryResults(); };
            buttonLast.Click += (s, e) => { currentPage = totalPages; LoadQueryResults(); };
            buttonGo.Click += (s, e) => 
    { 
        if (int.TryParse(textBoxPage.Text, out int page) && page > 0 && page <= totalPages)
        {
            currentPage = page;
            LoadQueryResults();
        }
        ...
        {
            MessageBox.Show($"請輸入有效的頁碼 (1-{totalPages})", "錯誤", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            textBoxPage.Text = currentPage.ToString();
        }
    };

    // 將事件綁定移至此處，確保在表單生命週期中只執行一次
    ...
    SetupDataGrid();
        ...
        {
            // 更新分頁信息
            labelPageInfo.Text = $"第 {currentPage} 頁，共 {totalPages} 頁 | 共 {totalRecords} 筆記錄";
            textBoxPage.Text = currentPage.ToString();
        ...
        private void LoadQueryResults(string query = null, bool isRepairOrder = false)
        {
            ...
            // 取得總記錄數
            string countQuery = $"SELECT COUNT(*) FROM ({currentQuery}) AS TotalCount";
            ...
            try
            {
                ...
                {
                    connection.Open();
                    using (var command = new System.Data.SQLite.SQLiteCommand(countQuery, connection))
                    {
                ...
                {
                    connection.Open();
                    using (var adapter = new System.Data.SQLite.SQLiteDataAdapter(pagedQuery, connection))
                    {
                        DataTable dt = new DataTable();
                        adapter.Fill(dt);
            ...
            if (dataGridViewQuery.Columns[e.ColumnIndex] is DataGridViewButtonColumn)
            {
                ...
                if (dataGridViewQuery.Columns[e.ColumnIndex].Name == "Edit")
                {
                    ...
                    if (Application.OpenForms.OfType<RepairUpdateForm>().Any())
                    {
                        ...
                        existingForm.Focus();
                        return;  // 添加這個 return 語句，防止繼續執行後面的代碼
                    }
                    EditRecord(id);
                }
        ...
        private void EditRecord(string id)
        {
            ...
            else
            {
                ...
                {
                    var result = updateForm.ShowDialog(this); // 將父表單傳入，使其居中顯示

                    // 清除選取狀態，防止在重新載入資料時重複觸發 CellClick 事件
